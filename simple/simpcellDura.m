function simpcellDura(mouse, date, run, saveraw, overwrite)
...
% copied to a home computer and read in easily.
   ...
   % Load the rotary encoder running data, if possible
    ...
    [active_mask, passive_mask, deriv_mask, pixel_mask] = replayProbableTimes(mouse, date, run);
    
    % Copy over registration motion
    shearml = mot.shearml;
    shearap = mot.shearap;
    transml = mot.transml;
    transap = mot.transap;
    ...
    if isempty(dirs.ml)
        ...
        % licking = ons.licking;
        if saveraw
            save(spath, 'dff', 'raw', 'centroid', 'brainmotion', ...
                'raw_max', 'dff_max', 'dff_median', 'dff_noise', 'dff_sigma', ...
                'deconvolved', 'running', 'framerate', 'edges', 'pupil', ...
                'speed', 'active_mask', 'passive_mask', 'deriv_mask', 'pixel_mask', ...
                'shearml', 'shearap', 'transml', 'transap', 'scaleml', 'scaleap');
        else
        ...
        if saveraw
            save(spath, 'dff', 'raw', 'centroid', 'brainmotion', 'raw_max', ...
                ...
                'codes', 'pupil', 'speed', 'active_mask', 'passive_mask', ...
                'deriv_mask', 'pixel_mask');
        else
            save(spath, 'dff', 'centroid', 'brainmotion', 'raw_max', ...
                'dff_max', 'dff_median', 'dff_noise', 'dff_sigma', ...
                'deconvolved', 'running', 'framerate', 'edges', 'onsets', ...
