function sbxPreprocessDura(mouse, date, runs, target, pcs, pmt)
...
%   Detailed explanation goes here
    ...
    alpars = struct('tbin', 0, 'highpass_sigma', -1);
    sbxAlignAffine(mouse, date, runs, target, pmt, alpars);
    ...
    for r = 1:length(runs)
        ...
        sbxSaveAlignedSBX(path, 0);
        sbxFirstLast(mouse, date, runs(r), 500, pmt);
        
        %if ~isempty(pcs), sbxPCACleanNoDS(path, pcs, pmt); end
        
        info = sbxInfo(path);
        
        if info.nchan > 1, sbxSaveAlignedSBX(path, 1); end
