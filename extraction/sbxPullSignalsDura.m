function sbxPullSignalsDura(mouse, date, runs, use_cleaned)
...
%   run simplifycellsort
    ...
    if nargin < 4, use_cleaned = true; end
    ...
    for i = 1:length(runs)
        ...
        if ~exist(sigfile)
            ...
            if isempty(cleanpath) || ~use_cleaned
                ...
                cellsort = sbxPullSignalsCoreCleaned(cleanpath, icaguidata);
            end
        ...
        % sbxStimulusTiff(mouse, date, run);
    end
    
    % Make a masked stimulus TIFF movie
    % sbxAlignmentMaskCheck(mouse, date);
end
